...
const Board = ({ title, posts, setTarget, setRoot }) => {
  const { loadPostsLoading, loadPostsDone } = useSelector(
    (state) => state.post,
  );
  ...
  return (
    <section className={styles.contentsWrapper}>
      ...
      <div className={styles.listContents} ref={setRoot}>
        {posts
      ...
      {loadPostsLoading && <LoadingIcon height='5rem' />}
    </section>
  );
};
